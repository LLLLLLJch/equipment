...
<body>
	...
	<input type="button" value="批量删除" onclick="deleteAll();" class="btn btn-default"/>
		<table class="table table-bordered">
			...
			<tr>
				<td>
					<input type="checkbox" name="selectedIds" value="${member.ID}">
				</td>
				<td><a href="javascript:showStudent('${member.ID}');">${member.name}</a></td>
				<td>${member.age}</td>
				<td>${member.gender}</td>
				<td>${member.workType}</td>
				<td><a href="javascript:update('${member.ID}')">修改</a></td>
				<td><a href="javascript:goDeleteStudent('${member.ID}');">删除</a></td>
			</tr>
			</c:forEach>
		</table>
	</form>

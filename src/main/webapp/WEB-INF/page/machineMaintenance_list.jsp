...
<script>
	...
	function showMaintenance(ID){
		layer.open({
	  	      ...
	  	      maxmin: true, //开启最大化最小化按钮
	  	      area: ['600px', '500px'],
	  	      content: "${pageContext.request.contextPath}/maintenanceIndex/showMaintenance.action?ID="+ID
	  	    });
	}
</script>
</head>
<body>
	...
	<input type="button" value="批量删除" onclick="deleteAll();" class="btn btn-default"/>
		<table class="table table-bordered">
			...
			<c:forEach items="${pageBean.list}" var="machineMaintenance">
			<tr>
				<td>
					<input type="checkbox" name="selectedIds" value="${machineMaintenance.id}">
				</td>
				<td><a href="javascript:showMaintenance('${machineMaintenance.id}');">${machineMaintenance.machineName}</a></td>
				<td>${machineMaintenance.content}</td>
				<td>${machineMaintenance.maintenanceDate}</td>
				<td>${machineMaintenance.maintenancePeople}</td>
				<td>
				...
				<td><a href="javascript:update('${machineMaintenance.id}')">修改</a></td>
				<td><a href="javascript:goDeleteStudent('${machineMaintenance.id}');">删除</a></td>
			</tr>
			</c:forEach>
		</table>
	</form>
